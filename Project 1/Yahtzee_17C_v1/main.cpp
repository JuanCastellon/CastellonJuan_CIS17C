...
void first(vector<Player>,int,vector<Dice>,int);   //Determining who goes first
...
void first(vector<Player> plyr,int nPlyr,vector<Dice> dice,int nDice){
    ...
    for(int i=0;i<nPlyr;i++){
        ...
        dice[0].roll();//Roll the dice
        indx1=dice[0].sum();   //Set the index of the player to sum to later
        plyr[i].setIndx(indx1);//rank the players based of the index (sum)
